...
It reads DB credentials from `backend/.env` automatically. Set `DATABASE_URL`
for a hosted PostgreSQL database, or use the same `DB_HOST`, `DB_PORT`,
`DB_NAME`, `DB_USER`, and `DB_PASSWORD` values as the Node backend. Override
the port with `REPORT_PORT`.
...
| GET | `/reports/staff/<name>` | Stable staff-app summary generated by Pandas |
| GET | `/reports/<name>.csv` | Download the underlying table as CSV |

Staff report names are `appointments`, `queue`, `cancellations`, `payments`,
